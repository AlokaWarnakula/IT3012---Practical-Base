...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, num_traps=5, custom_walls=None):
        ...
        # Declare a new hazard: toxic traps hidden in the environment, avoiding
        # the agent's start tile, walls, and food positions
        self.toxic_traps = set()
        while len(self.toxic_traps) < num_traps:
            tx = random.randint(0, self.width - 1)
            ty = random.randint(0, self.height - 1)
            trap_pos = (tx, ty)
            if (trap_pos != (0, 0) and trap_pos not in self.walls
    ...
    def get_percept(self) -> dict:
        ...
        off_grid = not (0 <= front[0] < self.width and 0 <= front[1] < self.height)
        return {
    ...
    def execute_action(self, action: str):
        ...
        # Lab 2 action model: 'Left'/'Right' TURN in place; 'Forward' steps one cell.
        ...
        if action == 'Left':
            ...
            on_grid = 0 <= new_pos[0] < self.width and 0 <= new_pos[1] < self.height
            if on_grid and tuple(new_pos) not in self.walls:
                self.agent_pos = new_pos
            elif on_grid and tuple(new_pos) in self.walls:
    ...
    def is_done(self) -> bool:
        return len(self.food_positions) == 0 or self.steps >= 1000 or self.collision #step count
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        for tx, ty in self.env.toxic_traps:
            ...
            self.canvas.create_rectangle(x1, y1, x1 + self.cell_size * 0.5, y1 + self.cell_size * 0.5, fill="#7c3aed",
                                         outline="#4c1d95")
    ...
    def run_loop(self):
        ...
        def step():
            if not self.env.is_done():
                percept = self.env.get_percept()          # SENSE (partial observability)
                action = self.agent.sense_and_act(percept)  # THINK (condition-action rules)
                self.env.execute_action(action)             # ACT
